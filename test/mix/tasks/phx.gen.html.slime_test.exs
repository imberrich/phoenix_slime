  ...
  test "generates html resource" do
    in_tmp "generates html resource", fn ->
      ...
      assert_file "lib/phoenix_slime_web/templates/user/form.html.slime", fn file ->
        assert file =~ ~s(= text_input f, :name)
        assert file =~ ~s(= number_input f, :age)
        assert file =~ ~s(= number_input f, :height, step: "any")
        assert file =~ ~s(= checkbox f, :famous)
        assert file =~ ~s(= datetime_select f, :born_at)
        assert file =~ ~s(= text_input f, :secret)
        assert file =~ ~s(= label f, :name)
        assert file =~ ~s(= label f, :age)
        assert file =~ ~s(= label f, :height)
        assert file =~ ~s(= label f, :famous)
        assert file =~ ~s(= label f, :born_at)
